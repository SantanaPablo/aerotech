...
import { Printer, FileText, Loader2, User, AlertTriangle, Calendar, Send, Settings, CheckCircle2 } from 'lucide-react';
...
const VerNotaSalida = ({ usuario }) => {
    ...
    const handleAprobar = async () => {
        setAprobando(true);
        try {
            await apiPatch(`/api/NotasSalida/${notaId}/aprobar`);
            await loadData();
        } catch (err) {
            console.error("Error al aprobar:", err);
            setError(err.message || 'Error al aprobar la nota.');
        } finally {
            setAprobando(false);
        }
    };
            ...
            <div className="p-8 bg-gray-50 min-h-screen">
                <div className="p-6 bg-white rounded-xl border border-red-300">
                    ...
                    <p className="mt-2 text-gray-700">
                        {error ? `No se pudo cargar la nota (ID: ${notaId}).` : `La nota de salida con ID **${notaId}** no existe.`}
                    </p>
    ...
    const estaAprobada = Boolean(notaData.recibido ?? notaData.Recibido);

    return (
        <div className="print-content2 p-6 sm:p-10 bg-white rounded-xl border border-gray-100 relative">
            <div className="no-print flex items-center gap-4 mb-8 flex-wrap">
                {/* Botón de Imprimir */}
                <button
                    className="flex items-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full transition duration-300 transform hover:scale-[1.02] active:scale-100"
                    onClick={handlePrint}
                >
                    <Printer className="w-5 h-5 mr-3" /> Imprimir Nota de Salida
                </button>

                {/* Botón de Aprobar (Solo Admin) */}
                {esAdmin && !estaAprobada && (
                    <button
                        onClick={handleAprobar}
                        disabled={aprobando}
                        className="flex items-center bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white font-bold py-3 px-8 rounded-full transition duration-300 transform hover:scale-[1.02] active:scale-100"
                    >
                        <CheckCircle2 className="w-5 h-5 mr-2" />
                        {aprobando ? 'Aprobando...' : 'Aprobar Nota'}
                    </button>
                )}
                ...
                {estaAprobada ? (
                    ...
                    <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-semibold bg-yellow-100 text-yellow-800">
                        <i className="bi bi-clock-fill"></i> Pendiente de Aprobación
                    </span>
                )}
            </div>
